 ...
 * @author steven
 */
public class RefrashSearchList extends Thread {
	...
	private Frm_SearchList parentFrame;

	public Frm_SearchList getParentFrame() {
		return parentFrame;
	}

	public void setParentFrame(Frm_SearchList parentFrame) {
	...
	protected RefrashSearchList(javax.swing.JTable tab, long time,
			...
			while (isRunning) {
				try {
					...
					if (rs.next()
						...
						RefrashSearchList.sleep(m_Time);
						continue;
					}
	...
	public void getSelectDate(String date, String severity, String finished) {
		sql = "SELECT A.visits_no AS '"
				...
				+ "WHERE "
				+ "case_manage.reg_guid = A.guid "
				+ "AND case_manage.p_no = patients_info.p_no "
				+ "AND case_manage.s_no = staff_info.s_no "
				+ "AND A.p_no = patients_info.p_no "
				+ "AND case_manage.finish_time LIKE '"
				+ date
				+ "%' "
				+ "AND case_manage.type = '"
				+ caseType
				+ "' "
				+ "AND case_manage.status = '"
				+ finished
				+ "' "
				+ (severity.isEmpty() ? "" : "AND case_manage.severity = '"
						+ severity + "' ")
				+ "ORDER BY case_manage.finish_time desc";
		try {
			rs = DBC.executeQuery(sql);
