...
public class DateChooser extends JPanel {
	...
	private String colname[] = { "Sun", "Mon", "Tue", "Wed", "Thu", "Fri",
			"Sat" };
	...
	class DatePanel extends JPanel implements MouseListener, ChangeListener {
		...
		private void initButton() {
			...
			for (int i = 0; i < 6; i++) {
				for (int j = 0; j < 7; j++) {
					...
					numberButton.setPreferredSize(new Dimension(
		...
		private void flushWeekAndDayPanal(Calendar c) {
			...
			for (int i = 0; i < 7; i++) {
				...
				cell.setPreferredSize(new Dimension((width - 30) / 7,
						(height - 30) / 7));
				cell.setOpaque(true);
				if (i == 0) {
	...
	class NewJPanelButton extends JButton {
