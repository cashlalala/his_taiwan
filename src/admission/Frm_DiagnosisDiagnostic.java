...
public class Frm_DiagnosisDiagnostic extends javax.swing.JFrame {
	...
	private String[] line = new String(
			paragraph.setlanguage("DIAGNOSISDIAGNOSTIC")).split("\n");
	...
	public void setTabRecord() {
		...
		try {
			...
			String sqlRecord = "SELECT shift_table.shift_date AS '"
					...
					+ "WHEN '2' THEN 'Noon' "
					+ "WHEN '3' THEN 'Night' "
					+ "WHEN '4' THEN 'All Night' END '"
					+ paragraph.getLanguage(line, "SHIFT")
					+ "', "
			...
			if (tab_Record.getColumnCount() == 6) {
				TableColumn columnNo = this.tab_Record.getColumnModel()
						.getColumn(0);
				TableColumn columnDate = this.tab_Record.getColumnModel()
						.getColumn(1);
				TableColumn columnShift = this.tab_Record.getColumnModel()
						.getColumn(2);
				TableColumn columnPoli = this.tab_Record.getColumnModel()
						.getColumn(3);
				TableColumn columnDoctor = this.tab_Record.getColumnModel()
						.getColumn(4);
			...
			// Logger.getLogger(Frm_DiagnosisDiagnostic.class.getName()).log(Level.SEVERE,
			// null, e);
			ErrorMessage.setData(
					"Diagnosis",
					"Frm_DiagnosisDiagnostic",
	...
	public void setTab_Diagnosis() {
		...
		try {
			...
			String sqlDiagnosis = "SELECT diagnosis_code.icd_code AS '"
					+ paragraph.getLanguage(line, "ICDCODE")
					+ "', diagnosis_code.name AS 'Item'"
			...
			if (rsDiagnosis.next()) {
				this.tab_Diagnosis.setModel(HISModel
						.getModel(rsDiagnosis, true));
			} else {
				((DefaultTableModel) tab_Diagnosis.getModel()).setRowCount(0);
			}

			// 設定寬度與最小寬度
			TableColumn columnNo = this.tab_Diagnosis.getColumnModel()
					.getColumn(0);
			TableColumn columnIcdCode = this.tab_Diagnosis.getColumnModel()
					.getColumn(1);
			TableColumn columnName = this.tab_Diagnosis.getColumnModel()
					.getColumn(2);
			...
			try {
				DBC.closeConnection(rsDiagnosis);
			} catch (SQLException e) {
				ErrorMessage.setData(
						"Diagnosis",
						"Frm_DiagnosisDiagnostic",
						"setTab_Diagnosis() - DBC.closeConnection",
						e.toString().substring(
								e.toString().lastIndexOf(".") + 1,
								e.toString().length()));
			}
	...
	public void settxta_Summary() {
		...
		try {
			...
			try {
				DBC.closeConnection(rsSummary);
			} catch (SQLException e) {
				ErrorMessage.setData(
						"Diagnosis",
						"Frm_DiagnosisDiagnostic",
						"settxta_Summary() - DBC.closeConnection",
						e.toString().substring(
								e.toString().lastIndexOf(".") + 1,
								e.toString().length()));
			}
	...
	public void setTab_Prescription() {
		...
		try {
			...
			String sqlPrescription = "SELECT prescription.code AS '"
					+ paragraph.getLanguage(line, "CODE")
					+ "', prescription_code.name AS 'Item' "
			...
			if (rsPrescription.next()) {
				this.tab_Prescription.setModel(HISModel.getModel(
						rsPrescription, true));
			} else {
				((DefaultTableModel) tab_Prescription.getModel())
						.setRowCount(0);
			}

			// 設定寬度與最小寬度
			TableColumn columnNo = this.tab_Prescription.getColumnModel()
					.getColumn(0);
			TableColumn columnPrescriptionCode = this.tab_Prescription
					.getColumnModel().getColumn(1);
			TableColumn columnPrescriptionName = this.tab_Prescription
			...
			try {
				DBC.closeConnection(rsPrescription);
			} catch (SQLException e) {
				ErrorMessage.setData(
						"Diagnosis",
						"Frm_DiagnosisDiagnostic",
						"setTab_Prescription() - DBC.closeConnection",
						e.toString().substring(
								e.toString().lastIndexOf(".") + 1,
								e.toString().length()));
			}
	...
	public void setTab_Medicines() {
		...
		String[] medicineTitle = { " ", "Code", "Item", "Dosage",
				...
				"Duration", paragraph.getLanguage(line, "QUANTITY"),
				paragraph.getLanguage(line, "URGENT"), "Powder",
				paragraph.getLanguage(line, "PS") };
		...
		try {
			...
			if (rsMedicines.last()) {
				...
				while (rsMedicines.next()) {
					for (int i = 0; i < medicineTitle.length; i++) {
						switch (i) {
						case 0:
							medicineArray[rsMedicines.getRow() - 1][i] = rsMedicines
									.getRow();
							break;
						default:
							medicineArray[rsMedicines.getRow() - 1][i] = rsMedicines
			...
			TableColumn medicineColumnNo = tab_Medicine.getColumnModel()
					.getColumn(0);
			...
			TableColumn medicineColumnPowder = tab_Medicine.getColumnModel()
					.getColumn(10);
			TableColumn medicineColumnPs = tab_Medicine.getColumnModel()
					.getColumn(11);
						...
						.addGroup(
								pan_UpLayout
										...
										.addComponent(
												...
												javax.swing.GroupLayout.PREFERRED_SIZE)
										.addGap(24, 24, 24)));
		pan_UpLayout.setVerticalGroup(pan_UpLayout.createParallelGroup(
	...
	private void tab_RecordMouseClicked(java.awt.event.MouseEvent evt) {// GEN-FIRST:event_tab_RecordMouseClicked
		if (tab_Record.getColumnCount() != 1
	...
	private void cob_PoliclinicItemStateChanged(java.awt.event.ItemEvent evt) {// GEN-FIRST:event_cob_PoliclinicItemStateChanged
		if (evt.getStateChange() == java.awt.event.ItemEvent.SELECTED
			...
			((DefaultTableModel) this.tab_Prescription.getModel())
					.setRowCount(0);
